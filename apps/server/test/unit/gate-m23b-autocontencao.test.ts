 ...
 *  (fora_do_filtro → entidade_vaga → nao_ancorado/C4 → nao_ancorado/âncora). Molde:
...
describe("M23-B — gate de auto-contenção (cadeia de gates, ordem cravada)", () => {
  it("1. claim vago ANCORADO sob filtro → entidade_vaga, approved vazio, id determinístico", () => {
    const merged = {
  ...
  it("2. ORDEM 1×2: claim vago em dimensão FORA do filtro → fora_do_filtro (a dim vence)", () => {
    const merged = {
  ...
  it("7. filtro=null → pass-through MESMA referência (golden intacto)", () => {
