/** UNIT — expandRecipeEntries (PURA, sem DB): o merge filtro→pack grava sob o PAR {type cru,
 ...
 *  (getExtractSchema(home, page.type)) PRECISA receber as dims do filtro — senão a dim é
...
describe("expandRecipeEntries — par {cru, efetivo}", () => {
  ...
  it("invariante dos SEEDS reais (espelho do §6.2-c, sem DB): a chave EFETIVA de cada seed recebe as dims do filtro", () => {
    const seeds = [
      { nome: "gmail", seed: emailSourceSeed("x") },
      { nome: "planilha", seed: planilhaIngestor.sourceSeed!() },
      { nome: "erp", seed: { type: "erp", filtro: { fields: [{ dimension: "claims" }] } } }, // conta-azul
    ];
    for (const { seed } of seeds) {
      const dims = seed.filtro.fields.map((f: any) => f.dimension);
