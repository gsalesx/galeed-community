 ...
 *  add-dimension (filtro ganha a dim via caminho EXISTENTE + merge no pack), idempotência,
...
const BRAIN = "itest-m25a-filtro-lote";
const SLUG = "pag-filtro-m25a-lote";
...
const DECIDED_BY = "humano-m25a-filtro";
...
async function seed() {
  ...
  await e.upsertPage({
    ...
    content_hash: "hash-filtro-m25a", tags: [`src:${SOURCE_ID}`],
  });
  await e.addReviewItems([
    rev({ id: "rev-filtro-m25a-1", quote: QUOTE1, claim: { text: "a", entity: "cliente", context_quote: QUOTE1 } }),
    rev({ id: "rev-filtro-m25a-2", quote: QUOTE2, claim: { text: "b", entity: "onboarding", context_quote: QUOTE2 } }),
    rev({ id: "rev-filtro-m25a-na", quote: "frase ausente zzz", claim: { text: "c", entity: "x", context_quote: "frase ausente zzz" } }),
    // pendente de OUTRO grupo (não pode ser tocado pela ação observacoes):
    rev({ id: "rev-filtro-m25a-outro", dimension: "precos", quote: QUOTE1, claim: { text: "d", entity: "plano", context_quote: QUOTE1 } }),
  ]);
}

describe.skipIf(!hasDb())("M25-A — handlers da borda (filtro + grupos, DB real)", () => {
  beforeAll(async () => {
    await seed();
  });
  afterAll(async () => {
  ...
  it("1. GET grupos: grupo fora_do_filtro|observacoes com count 3, dimensao_no_filtro false, decisão exata", async () => {
    const groups = await listHypothesisGroupsHandler(BRAIN);
    const g = groups.find((x) => x.reason === "fora_do_filtro" && x.dimension === "observacoes");
    expect(g).toBeDefined();
    expect(g!.count).toBe(3);
    expect(g!.dimensao_no_filtro).toBe(false);
    expect(g!.source_id).toBe(SOURCE_ID);
    expect(g!.decisao).toBe(
      `As regras da fonte "Fonte Filtro M25A" não têm o tipo "observacoes".`,
    );
    expect(g!.amostra.length).toBeLessThanOrEqual(3);
  });

  it("2. ação (c): filtro ganha 'observacoes' + 2 aprovados/1 retido; pack mergeado; outro grupo intocado", async () => {
    const r = await addDimensionGroupHandler(BRAIN, { source_id: SOURCE_ID, dimension: "observacoes" }, DECIDED_BY);
    expect(r.filtro_atualizado).toBe(true);
    expect(r.aprovados).toBe(2);
    expect(r.retidos).toHaveLength(1);
    expect(r.retidos[0].id).toBe("rev-filtro-m25a-na");

    const e = await getEngine(BRAIN);
    const src = await e.getSource(SOURCE_ID);
    const campo = src!.filtro.fields.find((f) => f.dimension === "observacoes");
    ...
    expect((await e.getReviewItem("rev-filtro-m25a-outro"))!.status).toBe("pendente");
  });

  it("3. idempotência da (c): repetir = no-op no EFEITO — filtro_atualizado false, aprovados 0, mesmo retido", async () => {
    ...
    const r = await addDimensionGroupHandler(BRAIN, { source_id: SOURCE_ID, dimension: "observacoes" }, DECIDED_BY);
    expect(r.filtro_atualizado).toBe(false);
  ...
  it("4. validações da borda: source_id '' → 400; dimension '' → 400; fonte inexistente → 404; reason inválido → 400", async () => {
    ...
    ).rejects.toThrowError(new BffError(400, "esse grupo não tem fonte — não há regras pra atualizar."));
    await expect(
      addDimensionGroupHandler(BRAIN, { source_id: SOURCE_ID, dimension: "" }, DECIDED_BY),
    ).rejects.toThrowError(new BffError(400, "diga o tipo pra adicionar às regras."));
    await expect(
  ...
  it("5. grupo esvaziado some: descartar o retido → o grupo observacoes sai do GET", async () => {
    const r = await discardGroupHandler(
      BRAIN, { reason: "fora_do_filtro", dimension: "observacoes", source_id: SOURCE_ID }, DECIDED_BY,
    );
    expect(r.descartados).toBe(1); // só o retido restava pendente
    const groups = await listHypothesisGroupsHandler(BRAIN);
    expect(groups.find((x) => x.reason === "fora_do_filtro" && x.dimension === "observacoes")).toBeUndefined();
  });
});
