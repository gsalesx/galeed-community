/** M22-C — sync Gmail do Galeed. BASE: template oficial NangoHQ/integration-templates@main
 *  integrations/google-mail/syncs/messages.ts (lido 2026-06-12). DESVIOS (2):
 *  (1) fetchMessage usa format:'full' (o template usa 'metadata'): o normalizador
 *      (src/core/ingestion/connectors/gmail.ts) precisa do payload com body.data —
 *      https://developers.google.com/workspace/gmail/api/reference/rest/v1/users.messages/get
 *  (2) syncBackfill aceita recorte opcional por data: se a connection metadata trouxer
 *      { backfill_after: "YYYY/MM/DD" }, o list ganha params.q = `after:YYYY/MM/DD`
 *      (operador de busca do Gmail — users.messages.list, param q). Ausente ⇒ backfill total
 *      (comportamento do template). Só leitura: escopo gmail.readonly.
 *
 *  Roda NO Nango (runtime 'nango' + 'zod' são do deploy, não dep do nosso repo). Incremental =
 *  historyId (decisão do CTO; o que o template oficial prova). Endpoint POST /syncs/messages,
 *  frequency 'every 5 minutes', autoStart true, modelo `Message` — IGUAIS ao template. */
...
async function syncBackfill(nango: NangoSyncLocal, checkpoint: NormalizedCheckpoint): Promise<void> {
    ...
    // DESVIO M22-C (2): recorte opcional por data via connection metadata { backfill_after: "YYYY/MM/DD" }.
    // best-effort (try/catch — metadata ausente ⇒ sem `q` ⇒ backfill total, comportamento do template).
    // Operador de busca do Gmail: users.messages.list, param q (https://developers.google.com/workspace/gmail/api/reference/rest/v1/users.messages/list)
    let after: string | undefined;
    try {
        const meta = (await nango.getMetadata()) as { backfill_after?: unknown } | null | undefined;
        const raw = meta && typeof meta.backfill_after === 'string' ? meta.backfill_after.trim() : '';
        if (raw) after = raw;
    } catch {
        after = undefined;
    }
...
async function fetchMessage(nango: NangoSyncLocal, messageId: string): Promise<z.infer<typeof MessageSchema> | null> {
    ...
    try {
        messageResponse = await nango.get({
            ...
            params: {
                // DESVIO M22-C (1): 'full' (não 'metadata') — o normalizador precisa do payload.body.data.
                format: 'full'
            },
            retries: 3
        });
